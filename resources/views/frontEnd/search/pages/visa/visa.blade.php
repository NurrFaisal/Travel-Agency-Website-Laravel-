...
@section('mainContent')
    ...
    <!--====== PLACES ==========-->
    <section>
        <div class="rows inn-page-bg com-colo" style="background-image: url({{asset('cosmos/frontEnd/visa/visa-back.jpg')}}); background-size: cover">
            <div style="background-color: transparent" class="container inn-page-con-bg tb-space pad-bot-redu" id="inner-page-title">
                ...
                <div>
                            @php
                            $visa_len = count($visas)
                            @endphp
                    <!--====== PACKAGE ==========-->
                    @if($visa_len != 0)
                    @foreach($visas as $con_visa)
                        @if($con_visa->status == 1)
                        <div class="col-md-3 col-sm-6 col-xs-12 b_packages zm wow fadeInUp">
                            <a href="{{URL::to('/visa-details/'.$con_visa->id)}}">
                                <div class="zm">
{{--                                    <div class="band"><img src="{{asset('cosmos/frontEnd/images/')}}" alt="" /> </div>--}}
