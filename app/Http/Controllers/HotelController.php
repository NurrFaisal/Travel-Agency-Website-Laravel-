...
//        $city = Division::with(['country' => function($q){ $q->select('id', 'name');}])->where('id', $id)->select('id', 'name', 'background_image', 'country_id')->first();
        $city = Country::where('id', $id)->select('id', 'name', 'banner_image')->first();
        $hotels = Hotel::where('country', $city->id)->paginate(10);
        $releted_hotels = Hotel::with(['countryf' => function($q){$q->select('id', 'name');}, 'divisionf' => function($q){$q->select('id', 'name');}])->where('country', $city->id)->select('id', 'name', 'box_image', 'country', 'division')->get();
