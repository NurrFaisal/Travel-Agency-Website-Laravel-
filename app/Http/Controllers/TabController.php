...
class TabController extends Controller
{
    ...
    public function viewPackageTab($id){
        $packagetab = PackageTab::with(['hotels.hotel' => function($q){ $q->select('id', 'name', 'star', 'category', 'web_site');},'tabItineraryTitles', 'package' =>function($q){$q->select('id');}])->where('id', $id)->first();
        $package_country = PackageCountry::where('package_id', $packagetab->package->id)->get();
        foreach ($package_country as $key => $package_country_id){
            $package_country_id_arry[$key] = $package_country_id->country_id;
        }
        $hotels = Hotel::whereIn('country', $package_country_id_arry)->select('id', 'name', 'star')->get();
